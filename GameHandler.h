...
public:
//punteros  a los demas objetos
  MouseHandler * mousehandler;
  ObjectsLauncher * objectslauncher;
  Texto * tex;
  vector<Mitad *> cortados;
  int nivel, frutas;
//el estado del juego si a terminado (en funcion a bomba)
  bool estado;          
...
// lanzador , verifica si en juego sigue o ya termino
  void run(){          
    if(!estado){
      objectslauncher->run();
...
// mueve los dos objetos resultantes del corte
  void mover_cortados(){     
  ...
  void detectar_corte(){
    ...
    if(mousehandler->puntos.size()){
      for (int j = 0; j < objectslauncher->frutas.size(); j++){
        if(distancia(mousehandler->puntos[mousehandler->puntos.size()-1], objectslauncher->frutas[j]->centro) <= objectslauncher->frutas[j]->tam){
      ...
      for (int j = 0; j < objectslauncher->bombas.size(); j++){
        if(distancia(mousehandler->puntos[mousehandler->puntos.size()-1], objectslauncher->bombas[j]->centro) <= objectslauncher->bombas[j]->tam){
          tex->estado = true;
          estado = true;
